...
setcookie('blab_acpkey',$acp_key,time()+3600*$settings['acp_lhours'],'/');
redirect('admin.php');}
...
if(!isset($_COOKIE['blab_acpkey']) || hsh($settings['acp_key'])!=$_COOKIE['blab_acpkey']){
$title=$lang['acpkey'];
...
setcookie('blab_acpkey','',time()+3600,'/');
redirect('admin.php');}

/* --- */

if(isset($_POST['bid']) && isset($_POST['bnm']) && isset($_POST['bip']) && isset($_POST['typ']) && isset($_POST['brs'])){

$typ=(int)$_POST['typ'];
$bid=(int)$_POST['bid'];
if(isset($_POST['per'])){$per=(int)$_POST['per'];}else{$per=0;}
$per=$timestamp+$per*60;

$brs=neutral_escape($_POST['brs'],255,'txt');
$bnm=neutral_escape($_POST['bnm'],64,'str');

if($typ>2){$bip=neutral_escape($_POST['bip'],50,'str');}else{$bip='';}

$query='INSERT INTO '.$dbss['prfx']."_ban VALUES($bid,'$bnm','$bip',$per,'$brs')";
neutral_query($query);

$query='DELETE FROM '.$dbss['prfx'].'_online WHERE usr_id='.$bid;
neutral_query($query);

redirect('admin.php?q=banned');}

if(isset($_GET['unban'])){
$unban=(int)$_GET['unban'];
$query='DELETE FROM '.$dbss['prfx'].'_ban WHERE ban_id='.$unban;
neutral_query($query);
redirect('admin.php?q=banned');}
...
$acp_timezone=neutral_escape($_POST['acp_timezone'],5,'int');
...
if(isset($_GET['reset_templates']) && is_file('./incl/styles.dat')){
$style=file('./incl/styles.dat'); 
for($i=0;$i<13;$i++){$style[$i]=trim($style[$i]);}
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$style[0]' WHERE set_id='slot_1d'"; neutral_query($query);
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$style[1]' WHERE set_id='slot_2d'"; neutral_query($query);
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$style[2]' WHERE set_id='slot_3d'"; neutral_query($query);
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$style[3]' WHERE set_id='slot_4d'"; neutral_query($query);
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$style[4]' WHERE set_id='slot_5d'"; neutral_query($query);
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$style[5]' WHERE set_id='slot_6d'"; neutral_query($query);
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$style[6]' WHERE set_id='slot_7d'"; neutral_query($query);
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$style[7]' WHERE set_id='slot_8d'"; neutral_query($query);
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$style[8]' WHERE set_id='slot_9d'"; neutral_query($query);
redirect('admin.php?q=style');}

if(isset($_POST['slotid']) && isset($_POST['slotve'])){
$slotid=neutral_escape($_POST['slotid'],8,'str');
$slotve=neutral_escape($_POST['slotve'],900,'str');
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$slotve' WHERE set_id='$slotid'"; neutral_query($query);
redirect('admin.php?q=style');
}
...
$new_user=abc123($_POST['add_user'],'');
$new_user=neutral_escape($new_user,60,'str');
$add_n='';
...
$new_mail=neutral_escape($new_user.'@'.$_SERVER['SERVER_NAME'],64,'str');
...
if(isset($_POST['legal_msg'])){

if(isset($_POST['legal_msg'])){
$legal_msg=neutral_escape($_POST['legal_msg'],65535,'txt');
$query='UPDATE '.$dbss['prfx']."_settings SET set_value='$legal_msg' WHERE set_id='legal_msg'";
neutral_query($query);}
...
if($int_version>$settings['int_version']){$q='update';}

switch ($q){
case 'online'   : $title=$lang['main'];     $page='sl_online.pxtm';break;
case 'iplog'    : $title=$lang['main'];     $page='sl_iplog.pxtm';break;
case 'banned'   : $title=$lang['main'];     $page='sl_banned.pxtm';break;
