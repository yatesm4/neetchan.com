iev=0;  lock_autoscroll=0; dragypos=0; dragdown=false; allowselect=0;
txt_b=0; txt_i=0; txt_c=999; tmp_j='';
...
function scrll(m){
dv=document.getElementById('blab_chat');
...
function nselect(b){
a=document.getElementById('blab_chat')
if(allowselect<1 && b>0){allowselect=1;a.style.cursor='auto'}
else{allowselect=0;a.style.cursor='ns-resize';
if(document.selection){document.selection.empty()} 
if(window.getSelection){window.getSelection().removeAllRanges()}}}
  
function mdown(x){if(allowselect<1 && typeof x.preventDefault=='function'){x.preventDefault()}dragdown=true;dragypos=x.pageY}
function mmove(x,y){if(dragdown==true && allowselect<1 && dragypos){x.scrollTop+=((dragypos-y.pageY)/6);
if(lock_autoscroll<1 && dragdown==true){close_l();set_opa('box_autoscroll',1);lock_autoscroll=1}}}
function w2move(x,y){if(parseInt(y.deltaY)>0){z=15}else{z=-15} x.scrollTop+=z}
...
function wmove(x,y){if(parseInt(y.deltaY)>0){z=15}else{z=-15} x.scrollTop+=z;
...
if(r[6]=='1'){window.location.href='ban.php';}
...
function force_dr(x){
if(chat_post.length>0){
nselect(0);lock_autoscroll=0;close_l();document.getElementById('box_autoscroll').style.display='none';document.getElementById('blab_chat').scrollTop=9999999;
...
a=document.getElementById('ln'); 
b=false;if(document.getElementById('pm')){b=document.getElementById('pm');}
document.getElementById('clr').style.backgroundColor='#'+c;
...
document.getElementById('box_paint_content').innerHTML='<iframe src="p_paint.php" scrolling="no" frameborder="0" style="margin:0;padding:0;width:400px;height:260px;border-width:0;overflow:hidden"></iframe>';
}}

function play_p(x){
close_l(); close_p(); set_opa('box_ins_paint_container',1); x=parseInt(x); 
ee='<iframe src="p_play.php?q='+x+'" scrolling="no" frameborder="0" style="width:400px;height:225px;border-width:0;overflow:hidden"></iframe>';
