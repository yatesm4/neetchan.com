...
$query='SELECT timestamp FROM '.$dbss['prfx']."_ban WHERE ban_ip='$ip' OR ban_id=$uid AND timestamp>$timestamp";
$result=neutral_query($query);
if(neutral_num_rows($result)>0){print '|-|0|0|0|0|1'; die();}
...
$line=url2link($line);
if(preg_match($paint_match,$line)){$line=str_replace("'",'&#39;',$line);$line=str_replace('\\','',$line);
$line=preg_replace($paint_match,"<img onclick=\"play_p('$1')\" class=\"paint_thumb\" src=\"paintings/$1.png\" alt=\"\" />",$line);}
...
else{if(stristr($line,'/me ') && !stristr($line,'http')){$line=str_replace('/me',$uuname,$line);}}
...
print $messages.'|'.$olist.'|'.$latest_id.'|'.$ohash.'|'.$queries.'/'.$total_time.'|'.$topic.'|0'; 
